...
// 층 증축
export const FLOOR_EXPANSIONS = [
  { floors:2, name:'2층 올리기', price:3000000, desc:'계단이 생기고 2층에 기계를 놓을 수 있다. 위층 손님은 계단으로 올라온다.' },
  { floors:3, name:'3층 올리기', price:8000000, desc:'옥상 직전까지. 3층은 조용해서 고급 손님이 좋아한다.' },
];
// 기계 고장 종류
...
export const BUBBLES = {
  win:  ['ㅁㅊ 한번에 나옴', '됐다!!!!', '개이득ㅋㅋㅋㅋ', '이집 뽑기 되네 ㄷㄷ', '사장님 사랑해요', '{item} 겟 ㅋㅋ', '{item} 내꺼', '아 손 떨려', '인생 첫 뽑기 성공', '이거 실화냐', '{item} 드디어…', '봤냐 봤냐'],
  lose: ['아 XX!!', '{item} 분명 잡았는데…', '한판만 더', '집게 뭐야 진짜', '내 돈…', '다음엔 됨', '{item} 거의 왔는데', '아 왜 놓쳐', 'ㅋㅋㅋ 망함', '천원만 더', '이게 안 잡혀?', '흑흑'],
  angry:['너무해!!!', '사장 나오라해', '돈먹는 기계네', '별점 1개 확정', '집게 힘 0임 ㅋㅋ', '이거 사기 아님?', '환불해줘요', '신고각'],
  toilet:['급하다 급해', '휴 살았다', '화장실 깨끗하네', '살았다…', '여기 화장실 개좋음'],
  toiletDirty:['으 화장실 냄새…', '여기 청소 안하나', '화장실 ㄹㅇ 개더러움', '숨 참고 들어감'],
  wanderItem:['{item} 있다!', '저 {item} 귀엽다', '{item} 얼마지', '{item} 뽑고싶다', '{item} 하나만…', '{item} 내 최애인데', '{item} 저거 진짜 큼', '{item} 여친 줘야지'],
  wanderEmpty:['인형이 하나도 없네', '여기 뭐 파는거야', '빈 기계만 있네', '재고 좀 채워요 사장님', '망한 가게인가', '텅텅 비었네 ㅋㅋ'],
  wanderBroken:['저거 고장 아니야?', '기계 불 꺼졌는데', '고장난거 언제 고치지', '저기 넣으면 돈 먹힘', 'ERROR 뜬거 봐'],
  wanderPricey:['{item} 너무 비싸다', '한판에 이 가격?', '가격 실화냐', '{item} 저거 다이소가 더 쌈', '양심 어디'],
  wanderCheap:['이 가격이면 해볼만한데', '{item} 싸다 싸', '여기 개혜자네', '{item} 저 가격 맞아?'],
  wanderCrowd:['사람 많네', '줄 서야겠다', '여기 핫플이네', '인스타에서 본 데다', '와 사람 뭐야'],
  wanderQuiet:['조용하네', '손님이 나밖에 없어', '망한 가게 아니지?', '분위기 좋다'],
  vending:['목마르다', '음료 하나', '콜라 있나'],
  stairs:['2층도 있네', '위층 가볼까', '엘베 없음?'],
};
export const CALLS = [
  '여보세요, 거기 {store}죠? 아까 {machine}에 돈 넣었는데 시작이 안 돼요. 환불해주세요.',
  '저기요, {machine} 돈만 먹고 안 움직이는데요? 3판 값 날렸어요.',
  '사장님 {machine} 고장 난 거 알고 계세요? 애가 울어요.',
  '{store} 맞죠? {machine} 앞에서 20분째 기다리는데 사장님 안 오세요? 돈 먹었다고요.',
  '아니 {machine} 동전 넣으면 삼키기만 하는데 이거 어떻게 할 거예요? 지금 영수증 찍어놨어요.',
];
...
// 리뷰 문구 ({item}, {machine} 치환). 커뮤·배달앱 리뷰 말투
export const NICKS = ['뽑기장인', '인형수집가', '동네주민', '지나가던행인', '뽑기초보', '집게분석가', '토끼덕후', '현금박치기', '오늘도꽝', '회오리마스터', '판다사랑', '월급루팡', '대학생A', '초딩아들엄마', '뽑기유튜버', '가챠중독', '아이패드원함', 'ㅇㅇ', '익명', '앙기모', '뽑기하는직장인', '집게의신', 'ㅂㅂ', '동네고인물', '인형탑쌓기', '군인휴가', '홍대감성', '헬창', '엄마몰래', '주식망함', 'kim_92', 'sooo_h', 'claw.lover', 'dduk_bokki', 'zzangu'];
export const REVIEWS = {
  win: ['{item} 한번에 뽑음 ㅋㅋㅋㅋ 여기 집게 ㄹㅇ 괜찮음', '{item} 득템!! 사장님 짱짱 ㅠㅠ', '3판만에 {item} 뽑았어요 인생가게 인정', '{item} 뽑고 기분좋아서 리뷰남김. 재방문의사 100%', '집게가 진짜로 잡아요 여기. 다른데는 손가락 힘 0인데', '{item} 뽑음. 여친이 좋아함. 이걸로 싸움 끝', '와 {item} 나왔다 이집 뽑기 잘됨 ㅇㅈ', '5천원 쓰고 {item} 두개 뽑음 흑자', '{item} 뽑았는데 봉제 상태 좋음 정품임', '뽑기 첨 해봤는데 {item} 나와서 개신남ㅋㅋ', '사장님이 팁 알려주심 {item} 겟', '여기 {item} 사이즈 큼. 가성비 ㅇㅈ'],
  lose: ['{item} 들었다가 떨어짐 ㅠㅠ 아깝', '오늘은 꽝. 근데 재밌어서 또 옴', '집게가 살짝 느슨한듯? 그래도 할만', '5천원 쓰고 빈손… 내일 또 감 ㅋㅋ', '{item} 노렸는데 옆에거만 밀림', '집게 힘 보통. 탑쌓기 하면 됨', '{item} 3번 들었는데 3번 다 상품구 앞에서 떨굼 ㅋㅋㅋ 약올리냐', '만원 썼는데 빈손. 근데 화는 안남 재밌음', '여기 집게 타이밍 맞추면 됨. 나만 못하는듯'],
  complaint: ['집게 너무해!!! 잡았다가 다 놓침', '돈 먹는 기계. 사장님 양심 어디', '여기 집게 힘 0임. 절대 가지마셈', '{item} 3번 들었다가 3번 다 떨어짐 이게 맞냐', '너무해 진짜 ㅡㅡ 별 하나도 아까움', '집게가 젓가락임 ㅋㅋㅋㅋ', '2만원 날림. 집게 세팅 뭐임?', '사장 얼굴 보고 싶다', '느슨하게 해놓고 장사하네', '여기서 뽑으면 기적임. 신고함'],
  gacha: ['가챠에서 {item} 나옴 개이득', '가챠 돌렸는데 {item}… 뭐 이 가격이면 ㅇㅈ', '재고처리 가챠 냄새나는데 {item} 나와서 용서', '가챠 {item} 당첨. 랜덤이라 짜릿함', '가챠 5번 돌려서 {item} 3개 ㅋㅋ 중복', '가챠 {item} 나옴. 손에 뭐라도 쥐고 가니 좋음'],
  rip: ['가챠 가격이 너무 비쌈. 바가지', '이 가격에 이 상품? 사기 아님?', '가챠 돌리고 현타옴. 다이소 가세요', '재고처리를 손님한테 하네 ㅋㅋ'],
  broken: ['{machine} 고장인데 방치중. 돈만 먹음', '기계 고장났는데 사장 없음. 별 1개', '동전 넣었는데 시작 안됨 ㅡㅡ', '{machine} ERROR 떠있는데 그냥 둠. 관리 안함', '고장난 기계에 돈 넣은 내가 바보', '{machine} 조이스틱 한방향으로만 감 ㅋㅋㅋ'],
  toilet: ['화장실 있는 뽑기집 처음봄. 개편함', '애랑 오기 좋음. 화장실 깨끗', '급똥 왔는데 화장실 있어서 살았다', '뽑기는 못했는데 화장실은 만족', '화장실 깨끗한 게 제일 큰 장점', '여기 화장실 때문에 옴 ㅋㅋ'],
  toiletBad: ['화장실 더러움… 청소좀', '화장실 상태 실화냐 숨 참고 들어감', '변기 안 닦은지 한달 된듯', '화장실 냄새가 매장까지 남. 청소 좀 하세요', '화장실 보고 나옴. 별점 깎음'],
  refund: ['돈 먹었는데 바로 환불해줌. 양심사장', '고장났다니까 3판값 돌려줌 ㅇㅈ', '전화하니까 바로 환불. 이런 가게 오래가야함'],
  ignore: ['돈 먹은거 전화했는데 씹음. 최악', '환불 안해주는 가게. 가지마세요', '바쁘다고 끊음 ㅋㅋㅋ 손님 돈은 안바쁘냐'],
  promo: ['SNS 보고 왔는데 사람 엄청 많네', '이벤트 한다길래 옴. 분위기 좋음', '유튜버 왔다갔대서 구경옴', '인증샷 올리면 1판 무료라길래 옴. 개꿀', '전단지 보고 옴. 근데 주차 어디함'],
};
// 리뷰 페르소나 (말투 다양화). 없는 종류는 기본 문구 사용
export const PERSONAS = [
  { id:'halbae', nicks:['김영감', '박노인', '동네할배', '경로당회장', '손주바보'], win:['허허 {item}이 딸려오는구먼. 손주 갖다줘야겄다', '이 나이에 뽑기를 다 하네. {item} 뽑았소이다', '집게가 참 야무지구먼. {item} 득', '젊은이들 노는 거 따라해봤는디 {item} 나왔어'], lose:['에이 이 집게가 힘이 없구먼', '내 젊었을 적엔 한번에 뽑았는디', '삼천원 넣고 헛손질만 했소', '손주가 하도 졸라서 왔는데 못 뽑았구먼 ㅠ'], complaint:['이거 사기 아니여? 잡았다 놓네', '사장 양반 집게 좀 조이시오', '노인 돈 뜯어먹는 기계구먼'], gacha:['이 뽑기통에서 {item}이 나왔소. 신기하구먼'] },
  { id:'jjaemin', nicks:['잼민이', '초딩킹', '롤하는초딩', '민준이', '서연이', '엄마몰래옴'], win:['{item} 뽑았다!!!!!! 엄마 나 천재인듯 ㅋㅋㅋㅋ', '오늘 학원 째고 옴 근데 {item} 뽑음 개꿀ㅋㅋㅋ', '님들 저 {item} 뽑음 ㅋㅋㅋ 부럽죠', '{item} 겟!! 친구가 부러워함 ㅎㅎㅎ', 'ㅋㅋㅋㅋㅋ {item} 한방컷'], lose:['아 ㅡㅡ 용돈 다 씀 ㅠㅠㅠ', '집게 빌런임 진짜', '엄마한테 혼남 5천원 씀', '{item} 잡았는데 떨어짐 ㅜㅜ 억울', 'ㅠㅠㅠㅠㅠ 다음주에 또옴'], complaint:['이거 사기임!!!! 신고할거임', '사장님 나빠요 ㅡㅡ', '집게가 손가락 하나 없는듯 ㅋㅋ 힘 0'], toilet:['화장실 있어서 다행 급했음 ㅋㅋ'] },
  { id:'mom', nicks:['○○맘', '쌍둥이맘', '동탄맘', '6세아들맘', '워킹맘'], win:['아이가 {item} 뽑고 너무 좋아하네요~ 사장님 감사해요♡', '{item} 뽑았어요! 애들 데리고 오기 좋아요 ㅎㅎ', '남편이 {item} 뽑아줌. 오랜만에 점수 땄네요', '동네 맘들한테 추천할게요~ {item} 겟!'], lose:['애들이 울어요 ㅠㅠ 집게가 좀 약한듯', '2만원 썼는데 빈손... 애 달래느라 힘들었네요', '재밌긴 한데 당첨은 어렵네요 ㅎㅎ'], complaint:['아이들 상대로 이러시면 안 되죠. 집게 힘 좀 올려주세요', '애가 울면서 나갔어요. 별로예요', '맘카페에 올릴게요. 집게 상태 심각'], toilet:['애랑 오기 좋아요~ 화장실 있음', '화장실 청소 좀 해주세요 ㅠㅠ 애 데리고 못 들어가겠어요'] },
  { id:'foreign', nicks:['Tom_in_Seoul', 'Yuki', 'Nguyen', 'Mike_traveler', 'Sasha'], win:['I got {item}!!! Korea claw machine best', '{item} 뽑았어요. 한국 인형뽑기 재미있어요!', 'wow {item}. my friend say impossible but I did', '여기 집게 strong 해요. {item} 겟'], lose:['집게 약해요... 만원 없어졌어요', 'so close!! {item} fell down ㅠㅠ', 'Korean claw machine very hard. but fun'], complaint:['this is scam?? claw no power', '사장님 집게 too weak. not fair'] },
  { id:'worker', nicks:['퇴근한직장인', '야근중', '대리님', '월요일싫어', '점심시간탈출'], win:['야근하고 오는길에 {item} 뽑음. 오늘 유일한 행복', '점심시간에 {item} 뽑아서 부장님 책상에 올려둠', '스트레스 풀러 왔다가 {item} 겟. 내일 출근 가능', '퇴근길 루틴됨. {item} 하나 추가'], lose:['월급 들어온 날 3만원 순삭', '회사보다 여기가 더 스트레스', '집게가 내 업무처럼 힘이 없음', '{item} 놓치고 현타와서 그냥 집감'], complaint:['사장님 집게 힘 좀 올려주세요. 회사에서 당하고 여기서도 당하네요', '돈 먹는 기계. 우리 회사 같음'] },
  { id:'otaku', nicks:['피규어수집러', '한정판헌터', '덕후생활', '굿즈지옥', '건담사랑'], win:['{item} 확보. 이건 컬렉션 각', '{item} 상태 A급. 박스 스크래치 없음. 만족', '드디어 {item}... 3주만에 겟', '{item} 뽑음. 이 가게 굿즈 라인업 괜찮음'], lose:['{item} 박스 모서리 걸려서 못 뽑음. 아쉽', '봉 위에서 아슬아슬... 다음에 밀어낸다', '박스 기스날까봐 조심하다 못뽑음'], complaint:['박스 상태 관리 좀 하세요. 찍힌 거 있음', '집게가 박스를 못 잡음. 세팅 재검토 바람'] },
  { id:'gym', nicks:['헬창', '3대500', '벌크업중', '닭가슴살', '오운완'], win:['{item} 원핸드로 뽑음. 오운완', '{item} 뽑음. 이것도 운동임', '3판만에 {item}. 집게도 근육이 있네'], lose:['집게 근손실 옴. 힘 0', '{item} 들었다가 떨어짐. 집게 벌크업 필요', '프로틴값 날림'], complaint:['집게 힘이 내 새끼손가락보다 약함', '사장님 집게 데드리프트 좀 시키세요'] },
  { id:'couple', nicks:['커플100일', '남친이뽑아줌', '여친선물', '우리커플', '데이트중'], win:['남친이 {item} 뽑아줌 ㅠㅠ 감동', '여친 주려고 {item} 뽑음. 성공', '100일 기념으로 {item} 겟 ♡', '데이트 코스로 굿. {item} 뽑아서 기분 좋았음'], lose:['남친이 5만원 쓰고 못뽑음. 싸움', '{item} 뽑아준다더니 빈손. 헤어질까', '데이트 망함. 집게 뭐임'], complaint:['커플 사이 갈라놓는 기계. 별점 1', '남친 돈 다 먹음 ㅡㅡ'] },
  { id:'soldier', nicks:['말년병장', '휴가나온군인', '이등병', '상병꺾인', '전역D-30'], win:['휴가 나와서 {item} 뽑음. 복귀 안하고싶다', '{item} 뽑아서 여친줌. 고무신 안 거꾸로 신겠지', '전역하면 여기서 알바한다. {item} 겟'], lose:['휴가비 다 씀. 복귀 어떡함', '집게가 이등병 팔힘', '{item} 놓치고 복귀함... 현타'], complaint:['휴가비 뜯어가는 기계. 국방부 신고', '집게 군기 잡아야됨'] },
];
